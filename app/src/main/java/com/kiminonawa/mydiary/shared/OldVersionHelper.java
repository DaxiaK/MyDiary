...
public class OldVersionHelper {
    ...
    public static void Version17MoveTheDiaryIntoNewDir(Context context) throws Exception {
        ...
        for (int i = 0; i < dataFiles.length; i++) {
            if (FileManager.isNumeric(dataFiles[i].getName())) {
                FileUtils.moveDirectoryToDirectory(dataFiles[i], new FileManager(context, FileManager.DIARY_ROOT_DIR).getDiaryDir(), true);
            }
        }
